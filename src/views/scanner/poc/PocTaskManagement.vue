            ...
            <v-col class="mt-n8 ml-4 mr-4" col="12">
                ...
                >
                    <template v-slot:item.name="{ item }">
                        <span @click="getTaskVul(item['tid'])" class="teal--text">{{item['name'] | long}}</span>
                    </template>
                    ...
                    <template v-slot:item.vul_count="{ item }">
                        <v-btn  x-small elevation="0" :color="item['vul_count'] === 0 ? 'grey lighten-3' : 'error'">
                            {{item['vul_count']}}
                        </v-btn>
                    </template>
                    ...
                    <template v-slot:item.action="{ item }">
                        ...
                        >
                            ...
                            <v-card>
                                <v-list>
                                    <v-list-item @click="exportTaskResult(item['tid'])">
                                        <v-list-item-action>
    ...
    export default {
        ...
        data: () => ({
            ...
            headers: [
                ...
                { text: 'Vul Count', value: 'vul_count' },
        ...
        methods: {
            getData() {
                this.$api.scanner.pocTaskList().then(res => {
            ...
            deleteTask(tid) {
                this.$api.scanner.pocTaskDelete(tid).then(res => {
                    let response = res.data;
                    let status = response['status'];
                    if(status['status'] === "success") {
            ...
            rescanTask(tid) {
                this.$api.scanner.pocTaskRescan(tid).then(res => {
                    let response = res.data;
                    let status = response['status'];
                    if(status['status'] === "success") {
                        this.$message.success(status['message']);
            ...
            exportTaskResult(tid) {
                window.open(this.$api.SERVER_ADDER + "/scanner/poc/export?keyword=tid&value=" + tid + "&token=" + this.$api.TOKEN)
            },
            getTaskVul(tid){
                window.open('#/scanner/poc/vul?tid=' + tid, "_blank");
            },
        }
    }
</script>
